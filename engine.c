	...
	while (1) {
		if (_kbhit()) {
			...
			if (key == 224) { // 방향키 입력
				...
				switch (key) {
				case 72: dir = d_up; break;  // 위
				case 80: dir = d_down; break;  // 아래
				case 75: dir = d_left; break;  // 왼쪽
			...
			else if (key == ' ') { //스페이스바로 선택
				display_status("Unit selected");
			}
		}
...
void init(void) {
	...
	for (int j = 0; j < MAP_WIDTH; j++) {
		map[0][0][j] = '#';
		map[0][MAP_HEIGHT - 1][j] = '#'; // 하단 벽
	}

	for (int i = 1; i < MAP_HEIGHT - 1; i++) {
		map[0][i][0] = '#'; // 좌측 벽
		map[0][i][MAP_WIDTH - 1] = '#'; // 우측 벽
...
void cursor_move(DIRECTION dir) {
	...
	case d_down:new_pos.row++;break;
	case d_left:new_pos.column--;break;
	case d_right:new_pos.column++;break;
	}
